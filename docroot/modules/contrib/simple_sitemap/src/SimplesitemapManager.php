...
class SimplesitemapManager {
  ...
  const VARIANT_INDEX_SITEMAP_TYPE = 'variant_index';
   ...
   * @return array
   */
  public function getSitemapVariants($sitemap_type = NULL, $attach_type_info = TRUE, $exclude_variant_index = FALSE) {
    if (NULL === $sitemap_type) {
      ...
      foreach ($this->configFactory->listAll('simple_sitemap.variants.') as $config_name) {
        ...
        if (!$exclude_variant_index || ($exclude_variant_index && $config_name_parts[2] != SimplesitemapManager::VARIANT_INDEX_SITEMAP_TYPE)) {
