...
class PasswordPolicy extends ConfigEntityBase implements PasswordPolicyInterface {
  ...
  public function calculateDependencies() {
    ...
    foreach ($this->getConstraints() as $constraint) {
      $plugin_definition = $plugin_manager->getDefinition($constraint['id']);
      $this->addDependency('module', $plugin_definition['provider']);
    }
  ...
  /**
   * Gets the password policy plugin manager.
   *
   * @return \Drupal\password_policy\PasswordConstraintPluginManager
   *   The plugin manager.
   */
